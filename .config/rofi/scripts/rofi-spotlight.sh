...
function find_query() {
    if [[ ! "${1}" =~ ( |\') ]]; then
        fd -H ${1} ${HOME} | sed "s/\/home\/$USER/\~/" | awk -v MY_PATH="${MY_PATH}" '{print ""$1"??\0icon\x1f"MY_PATH"/icons/result.svg"}'
    fi
...
function context_menu_icons() {
	...
	then
		echo "\0icon\x1fuser-trash\n"

	elif [[ "${1}" == "Delete" ]]
	then
		echo "\0icon\x1fdelete\n"
