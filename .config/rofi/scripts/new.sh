...
# Find argument
function find_query() {
    fd -H "${1}" "${HOME}" 2>/dev/null | sed "s/\/home\/$USER/\~/" | awk -v MY_PATH="${MY_PATH}" '{print $1"\0icon\x1f"MY_PATH"/icons/result.svg"}'
}
...
if [ -n "$*" ] && [[ "${ALL_OPTIONS[*]} " = *"$*"* ]]; then
	case "${1}" in
		"Run" )
			coproc "$(< "${CURRENT_FILE}")" > /dev/null 2>&1
			kill -9 "$(pgrep rofi)"
			;;
		"Execute in ${TERM_EMU}" )
			coproc "${TERM_EMU}" $(< "${CURRENT_FILE}") > /dev/null 2>&1
			kill -9 "$(pgrep rofi)"
			;;
		"Open" )
			coproc "${OPENER}" $(< "${CURRENT_FILE}") > /dev/null 2>&1
			kill -9 "$(pgrep rofi)"
			;;
		"Open file location in ${TERM_EMU}" )
		...
		"Open file location in ${FILE_MANAGER}" )
			file_path="$(< "${CURRENT_FILE}")"
			coproc "${FILE_MANAGER}" "${file_path%/*}" > /dev/null 2>&1
			kill -9 "$(pgrep rofi)"
			;;
		"Edit" )
			coproc "${TERM_EMU}" "${TEXT_EDITOR}" $(< "${CURRENT_FILE}") > /dev/null 2>&1
			kill -9 "$(pgrep rofi)"
			;;
		"Move to trash" )
